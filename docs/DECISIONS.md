...
## D-022 — "Recent joiners" is company-wide for every role, via its own reduced endpoint
**Context:** The dashboard's recent-joiners panel was derived from the scoped employee list, so an EMPLOYEE saw only themself and a MANAGER only their team. User: "in recent joiners for all the roles same recent added employees should be visible".
**Decision:** A new `GET /api/dashboard/recent-joiners?limit=` returns the newest ACTIVE joiners company-wide, identical for every role, projected to `id, firstName, lastName, department, designation, joiningDate` only. Its predicate is a policy function, `recentJoinersWhere(actor)`, written beside `scopeWhere` and covered by the policy unit tests, so the exception is visible in the one file that holds the access model. Nothing else changes: the register, the record lookup and `/dashboard/stats` keep `scopeWhere`. The frontend panel reads the new endpoint; the scoped list still feeds reporting lines and inactive accounts.
**Why:** This is a deliberate relaxation of CLAUDE.md rule 5 for one welcome-list feature, so it should be explicit and reviewable rather than a wider scope smuggled into the list endpoint. Keeping it in the policy file preserves "nothing outside `employeePolicy.ts` decides who sees what" and the graph check. The reduced projection means seeing a joiner here unlocks nothing: `GET /api/employees/:id` for that person still answers 403 to an EMPLOYEE (asserted in the integration test).
**Trade-off:** Every authenticated user now learns the names, titles, departments and start dates of the newest hires — announcement-level information in most companies, but information. If that is ever unwanted, the single switch is `recentJoinersWhere`.
