...
## AI-022 — Phone: exactly ten digits, enforced on both sides (2026-09-25)
**Prompt (summary):** User: "fix when updating number or adding number for new employee i can add the characters and number greater than 10 digits fix it".
**Generated / changed:**
- `backend/src/modules/employees/employees.schemas.ts` — `phone` is `^\d{10}$` ("Phone must be exactly 10 digits"), still nullable/optional on create and update. `openapi.yaml` `pattern` + ten-digit examples; `prisma/seed.ts` phones `98450000NN`; `postman/` PUT bodies; `tests/integration/{authorization,employees}.test.ts` literals plus five new 400 cases (country code, dashes, letters, 9 digits, 11 digits).
- `frontend/src/lib/phone.ts` — `normalizePhoneInput`: strip non-digits, drop a 12-digit `91…` prefix or 11-digit leading `0`, cut at ten. `components/FormField.tsx` — `PhoneField` (`type=tel`, `inputMode=numeric`, applies the normaliser on every input event). `pages/EmployeeForm.tsx`, `pages/MyProfile.tsx` — use it; Zod rule `^\d{10}$` or empty, message "Enter exactly 10 digits".
- README note under Endpoints; `docs/DECISIONS.md` D-020.
**Issues found:**
- First `PhoneField` carried a native `maxLength={10}`. In the browser, pasting `+91 (98450) abc-00011 22` produced `9198450` and a typed 17-character mixed string produced `98450`: the browser truncates the raw text to ten characters *before* the input handler strips it, so the `+91 ` counted and real digits were lost. Fixed by removing `maxLength` and letting the normaliser do the limiting; the `+91` / leading-`0` drop was added at the same time because the paste case made it obvious.
- My first bulk edit script died on a console encoding error after writing the schema and seed but before the tests and OpenAPI; caught by re-listing the old `+91-` literals and finishing the rest.
- The lint rule `react-refresh/only-export-components` warned when the helper lived in `FormField.tsx`; moved to `lib/phone.ts`.
**Human corrections:** None.
**Verification:** Backend `npx tsc --noEmit`, `npm run lint` clean, `npm run test:unit` 63/63. Frontend `tsc`, `lint`, `build` clean. Browser (local frontend → live Render API through a throwaway CORS proxy in the scratchpad, since the local backend still has the stale DB password): on `/me` as employee1 — typing `123456789012` → `1234567890`, `12a3` → `123`, `12-34 56` → `123456`, 9 digits + Save → "Enter exactly 10 digits", `9845011111` + Save → contact panel shows the new number; paste `+91 (98450) abc-00011 22` → `9198450000`, `098450-00001` → `9845000001`, `phone: 98450abc00001 ext 22` → `9845000001`. On `/employees/new` as admin — field is `tel`/`numeric` with no `maxLength`, paste normalised, 8 digits + Create → "Enter exactly 10 digits". **Not run:** integration tests (need DB consent and a current `.env.test` password); the live API still enforces the old rule until this is deployed. Cleanup: proxy stopped, `frontend/.env` restored, dev server restarted.
**Commit:** suggested — `fix(phone): exactly ten digits on the API, the forms and the seed`

## AI-023 — Catalog dropdowns, manager-required rule, letters-only names, UI hints, nav highlight (2026-09-25)
**Prompt (summary):** User listed six fixes: department/designation as fixed dropdowns; no employee without a manager except ADMIN; first/last name letters only; the "You may edit: firstName, lastName…" hint shows API field names — fix the casing and remove it on the HR edit page; the Register nav entry lights up while on Add employee.
**Generated / changed:**
- `backend/src/modules/employees/employees.catalog.ts` (new) — `DEPARTMENTS`, `DESIGNATIONS`, `requiresManager(role)`. `employees.schemas.ts` — `z.enum` for both lists, `NAME_PATTERN`, `MANAGER_REQUIRED_MESSAGE`, create-side refine. `employees.service.ts#update` — effective role/manager check after the policy gate. `openapi.yaml` — `Department`/`Designation` schemas, name patterns, `managerId` descriptions, 400 descriptions.
- `backend/tests/unit/employeesSchemas.test.ts` (new, 38 cases) — every catalog value accepted, outsiders rejected, name table, manager rule per role. `tests/integration/employees.test.ts` — default body now valid under the new rules (letters-only surname, catalog designation, `managerId: 'EMP011'`); 7 new 400 cases; ADMIN-without-manager 201; clear-manager split into EMPLOYEE → 400 / ADMIN → 200; demote-top-level-ADMIN → 400; `Ops` → `Marketing`. `authorization.test.ts` — `User${suffix}` → `'User'`, `'X'`/`'CTO'` → catalog values (they must reach the policy and answer 403, not fail validation first). `postman/` — Test 3 body made valid for the same reason.
- `frontend/src/lib/catalog.ts` (new mirror). `pages/EmployeeForm.tsx` — `SelectField` dropdowns, `NAME_PATTERN`, `superRefine` for the manager rule on both create and edit schemas, hints. `lib/permissions.ts` — `FIELD_LABELS` / `fieldLabel`. `pages/EmployeeDetail.tsx` — line hidden for ADMIN, human labels otherwise. `components/ErrorBanner.tsx` — details use the same labels. `App.tsx` — Register link is not active on `/employees/new`.
- `docs/DECISIONS.md` D-021; README notes.
**Issues found:**
- Order of middleware matters: Zod validation runs before the policy, so three existing authorization tests (`designation: 'X'`, `'CTO'`, `lastName: 'User<t3>'`) and the Postman Test 3 body would have started answering 400 instead of 403. Caught by reading the tests before changing the schema; bodies fixed to valid values so they still test authorization.
- Frontend is Zod 4, backend Zod 3: `z.enum(..., { errorMap })` is a type error on the frontend; `{ message }` there. Caught by `tsc`.
- Long bash heredoc failed again (known limit); script written to the scratchpad and run from there.
- In the browser, setting text-input values from JavaScript does not reach React Hook Form (React swallows programmatic value sets), which made the first name-rule check look like "Required". Re-ran with the form tool; rule confirmed. Select elements did accept programmatic change events.
**Human corrections:** None; all six items are the user's.
**Verification:** Backend `tsc`, `lint` clean; `npm run test:unit` **101/101** (63 + 38 new). `openapi.yaml` parses (yaml): Department enum 5, Designation enum 18. Frontend `tsc`, `lint`, `build` clean. Browser (local frontend → live API via scratchpad proxy): on `/employees/new` as admin — `department`/`designation` are `<select>`s (6 and 19 options incl. "— Choose —"); nav shows Register inactive / Add employee active; submitting `Neha2` / `Rao_` with no manager → "Letters only" ×2 + "Everyone except an admin must report to someone"; switching role to ADMIN removes the manager error. As manager on `/employees/EMP001` — "You may edit: Designation, Department. Anything else is rejected by the API." As admin on the same page — no such line. **Not run:** integration tests (DB password still stale in `.env.test`); the live API still runs the old rules until deployed. Cleanup: proxy stopped, `frontend/.env` restored, dev server restarted.
**Commit:** suggested — `feat(employees): catalog dropdowns, manager required unless ADMIN, letters-only names; humanise field hints; fix nav highlight`

## AI-024 — Recent joiners identical for every role (2026-09-25)
**Prompt (summary):** User: "fix one more thing in recent joiners for all the roles same recent added employees should be visible".
**Concern raised first:** this widens what an EMPLOYEE can see beyond CLAUDE.md rule 5, so it was built as an explicit, minimal exception rather than by loosening the list scope (D-022).
**Generated / changed:**
- `backend/src/policies/employeePolicy.ts` — `recentJoinersWhere(actor)` → `{ status: ACTIVE }` for every role, with the rationale in its doc comment and the file header.
- `backend/src/modules/dashboard/` — `dashboard.schemas.ts` (new: `limit` 1–20, default 5, `.strict()`), `dashboard.service.ts#recentJoiners` (narrow `select`, newest first, id desc tiebreak), `dashboard.controller.ts`, `dashboard.routes.ts` (`GET /recent-joiners` behind `authenticate` + `validate`). `openapi.yaml` — `RecentJoiner`, `RecentJoinersResponse`, the path with 400/401.
- Tests — `tests/unit/employeePolicy.test.ts` (+2: same predicate for every role; never narrows by actor id); `tests/integration/dashboard.test.ts` (+6: 401; identical bodies for admin/manager/employee; newest-first and matches the DB; only the six fields and no email/phone/role/status/managerId anywhere in the body; an EMPLOYEE sees people the register still hides and still gets 403 on their record; limit validation).
- Frontend — `types/api.ts#RecentJoiner`, `api/dashboard.ts#fetchRecentJoiners`, `pages/Dashboard.tsx` reads the new endpoint for the panel (eyebrow "Company"); `PersonRow` takes a `Pick` so the reduced shape fits. The scoped list still drives reporting lines and inactive accounts.
- `docs/AUTHORIZATION.md` (roles table note, policy function, endpoint row), README endpoint table, D-022.
**Issues found:**
- First attempt at reading the panel in the browser used `closest('section, div')` from the heading and hit the header wrapper (empty list); re-read via page text. Not a product issue.
- The browser tool's element refs reset after a full navigation mid-batch; re-found them and continued.
**Human corrections:** None.
**Verification:** Backend `tsc`, `lint` clean; `npm run test:unit` **103/103**; `openapi.yaml` parses with the new path. Frontend `tsc`, `lint`, `build` clean. Browser against a throwaway API double in the scratchpad (the local backend has no working DB password and the live API does not have the endpoint yet): as employee1 the panel lists Lakshmi Pillai, Divya Sharma, Ananya Bose, Arjun Iyer, Neha Kulkarni (newest first) while the rest of the dashboard stays scoped to one record; as admin the panel lists the same five. **Not run:** the six new integration tests and the rest of the DB suite (`.env.test` password stale) — they are written against the real service and will run with `npm test` once the password is updated. Cleanup: stub stopped, `frontend/.env` restored, dev server restarted.
**Commit:** suggested — `feat(dashboard): company-wide recent joiners for every role via a reduced endpoint (D-022)`

## AI-025 — Reporting lines: name the manager who is outside the caller's view (2026-09-25)
**Prompt (summary):** User (with screenshot): in the manager's dashboard, the Structure panel shows "EMP000 — outside your view" for the manager's own manager; it should show the name.
**Generated / changed:** `frontend/src/pages/Dashboard.tsx` — each reporting-line group now also carries `managerName`, taken from any report's `manager.name` (the employee DTO already includes the manager summary for every row, so no API change and no widening of scope). When the manager is not in the scoped list, the row shows their monogram, name and id instead of the bare id; the "outside your view" fallback remains only for a row whose reports carry no manager summary.
**Issues found:** None. The stub API double in the scratchpad initially returned `manager: null` on every row (unlike the real DTO); fixed the double, not the app.
**Human corrections:** None.
**Verification:** `tsc`, `lint`, `build` clean. Browser as manager against the API double: Structure panel reads "Rahul Verma — Engineering Manager — 2 reports" and "Asha Menon — EMP000 — 1 report" (was "EMP000 — outside your view"). Cleanup: stub stopped, `frontend/.env` restored, dev server restarted.
**Commit:** suggested — `fix(dashboard): show the manager's name on a reporting line whose manager is outside the caller's scope`
