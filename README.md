...
A [`render.yaml`](render.yaml) blueprint defines both services. In Render: **New → Blueprint**, pick the repo, and fill in the prompted variables.

| Service | Root | Build | Start |
|---|---|---|---|
| `hrms-api` (web, Node) | `backend` | `npm ci --include=dev && npm run build` | `npm run deploy` → [`scripts/deploy.cjs`](backend/scripts/deploy.cjs): logs host/port/params of both DB URLs, repairs a pooled URL missing `pgbouncer=true` and an IPv6-only `DIRECT_URL`, runs `prisma migrate deploy`, starts `dist/server.js` |
| `hrms-web` (static) | `frontend` | `npm ci && npm run build` | publishes `dist/` with a `/* → /index.html` rewrite |

Variables to provide: `DATABASE_URL` and `DIRECT_URL` (Supabase), `CORS_ORIGIN` (the frontend's Render URL), and for the frontend `VITE_API_URL` (the backend's Render URL + `/api`). `JWT_SECRET` is generated by Render. `PORT` is injected by Render and read automatically.

> **`DIRECT_URL` on Render must be the Supabase _Session pooler_ URL** (`aws-0-<region>.pooler.supabase.com:5432`, user `postgres.<project-ref>`), not the "Direct connection" host `db.<project-ref>.supabase.co`. That host is IPv6-only; Render's free tier has no outbound IPv6, and `prisma migrate deploy` fails with `P1001: Can't reach database server`. The session pooler is IPv4 and supports migrations. The deploy script detects this case on Render (`RENDER` is set, `DIRECT_URL` is the `db.` host, `DATABASE_URL` is on the pooler), derives the session-pooler URL from `DATABASE_URL`, logs a warning naming both hosts, and continues — so a wrong `DIRECT_URL` degrades to a warning instead of a failed deploy. Passwords are never logged.

If you configure the services by hand instead, the two things that matter:
...
**Cold starts.** Render's free tier stops the API after ~15 idle minutes; the first request afterwards can take up to a minute. The frontend handles this: on page load it pings `/health` so the server starts booting immediately, and the API client retries cold-start failures (no response, or a 502/503/504 that is not the API's own error body) up to **3 attempts** with 4 s and 10 s pauses, on **every route**. Writes are safe to repeat here: a platform 502/503 means the request never reached the app; on a timeout, `PUT` reapplies the same data, `DELETE` is a soft delete, and a repeated `POST /employees` is rejected by the unique email (409) rather than duplicated. While retrying, every page shows *"Waking the server — attempt n of 3"* (login page, session check, and the app shell). See `frontend/src/api/client.ts`.
...
- **Login works but `/employees` and `/dashboard/stats` return 500** → `DATABASE_URL` is the transaction pooler (port 6543) **without `?pgbouncer=true`**. Prisma then uses prepared statements, and any two queries run in parallel land on different pooled backends. The deploy script now adds the flag automatically for Supabase and logs `was missing pgbouncer=true`; the API logs the remedy and returns 503 instead of 500.
