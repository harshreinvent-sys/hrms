...
|---|---|---|---|
| `hrms-api` (web, Node) | `backend` | `npm ci --include=dev && npm run build` | `npm run deploy` → [`scripts/deploy.cjs`](backend/scripts/deploy.cjs): prints the DB hosts, runs `prisma migrate deploy`, starts `dist/server.js` |
